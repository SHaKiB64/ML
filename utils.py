...
def calculate_accuracies(results, labels=None, run_timestamp='unspecified'):
    """
    Calculate, display, and save various accuracy metrics after a test run. 
    This method is only for post-processing; Keras reports its built-in 
    accuracy calculations during training/validation runs.
    ...
    # Save classification report with error handling
    try:
    ...
    conf_matrix_path = os.path.join(params.RESULTS_FOLDER, conf_matrix_filename)
    df = pd.DataFrame(conf_matrix)

    # Save confusion matrix with error handling
    try:
...
def plot_train_metrics(model_history, model_name, results_folder, run_timestamp='unspecified'):
    ...
    Returns:
        ...
        acc_fig_path: string representing path to the saved accuracy plot for the run
    """
    ...
    train_acc = model_history.history.get('binary_accuracy') or model_history.history.get('accuracy')
    val_acc = model_history.history.get('val_binary_accuracy') or model_history.history.get('val_accuracy')
...
def save_model(model, model_history, model_name, results_folder, run_timestamp='unspecified'):
    ...
    try:
        ...
        os.remove(symlink_path)  # Remove if exists
        os.symlink(output_weights, symlink_path)
    except OSError as e:
